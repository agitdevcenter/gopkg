...
func (conn *connection) FindOneAndDelete(filter interface{}, outputVal interface{}, opts ...*Options) error {
	...
	if opts != nil {
		...
		if opts[0].CollectionName == "" {
			...
			res = conn.multiCollection[opts[0].CollectionName].FindOneAndDelete(conn.ctx, filter, Opts)
		}
	} else {
...
func (conn *connection) FindOneAndReplace(filter, replacement interface{}, outputVal interface{}, opts ...*Options) error {
	...
	if opts != nil {
		...
		if opts[0].CollectionName == "" {
			...
			res = conn.multiCollection[opts[0].CollectionName].FindOneAndReplace(conn.ctx, filter, replacement, Opts)
		}
	} else {
...
func (conn *connection) InsertMany(documents []interface{}, opts ...*Options) (insertedIDs []string, err error) {
	var result *mo.InsertManyResult
	startTime := time.Now()
	if opts != nil && opts[0].CollectionName != "" {
		result, err = conn.multiCollection[opts[0].CollectionName].InsertMany(conn.ctx, documents)
	} else {
...
func (conn *connection) ReplaceOne(filter, replacement interface{}, opts ...*Options) (upsertedID string, modifiedCount int64, err error) {
	...
	if opts != nil {
		...
		if opts[0].CollectionName == "" {
			...
			result, err = conn.multiCollection[opts[0].CollectionName].ReplaceOne(conn.ctx, filter, replacement, Opts)
		}
	} else {
