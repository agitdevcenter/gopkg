...
var sqlConns = &SQLConn{DB: make(map[string]*Client)}
...
func (s *SQLConn) Get(id string) *Client {
	s.mux.Lock()
	defer s.mux.Unlock()
...
//Set set connection
func (s *SQLConn) Set(id string, Client *Client) {
	s.mux.Lock()
	s.DB[id] = Client
	s.mux.Unlock()
}
...
func Connect(id string, opt *Options) (*Client, error) {
...
func connect(id string, opt *Options) (*Client, error) {
	...
	sqlConns.Set(id, &Client{DB: conn, logMode: opt.LogMode})
