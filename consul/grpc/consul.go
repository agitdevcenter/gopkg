...
func ConnectWithRoundRobinWithOption(cfg *ConsulConnect, option grpc.DialOption) (conn *grpc.ClientConn, err error) {
	SetupResolver()

	ctx, _ := context.WithTimeout(context.Background(), cfg.Timeout)
	conn, err = grpc.DialContext(ctx, cfg.Target, grpc.WithInsecure(), grpc.WithBalancerName("round_robin"), option)
	return
}
